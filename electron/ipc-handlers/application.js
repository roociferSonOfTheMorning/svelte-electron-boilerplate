...
const letterDatabase = require("../collections/letterDB");
...
ipc.handle("getLetters", async (event, ...args) => {
  const result = await letterDatabase.getDataCollection(...args);
  return result;
});

ipc.handle("loadLetters", async (event, ...args) => {
  const result = await letterDatabase.loadDataCollection(...args);
  return result;
});
